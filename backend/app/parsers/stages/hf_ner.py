...
class HuggingFaceNERStage(BaseParserStage):
    def run(self, document: BaseDocument, context: PipelineContext) -> None:
        ...
        if not hf_pipeline:
            context.log_warning(
                "HFLoadError", "HF pipeline not loaded, skipping stage."
            )
            return
        ...
        for offset, chunk in chunks:
            ...
            for res in hf_results:
                label = res.get("entity_group") or res.get(
                    "entity"
                )  # some pipelines return entity_group for simple aggregation
        ...
        document.metadata["ai_inference_time_ms"] = (
            document.metadata.get("ai_inference_time_ms", 0) + duration
        )
        if "model_versions" not in document.metadata:
            document.metadata["model_versions"] = {}
        document.metadata["model_versions"]["hf_ner"] = getattr(
            hf_pipeline, "name", "dslim/bert-base-NER"
        )
