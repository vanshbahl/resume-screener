...
from parser_tests.framework.metrics import (calculate_field_match,
                                            calculate_list_metrics,
                                            evaluate_complex_list)


def load_expected_output(
    resume_filename: str, expected_dir: str = "parser_tests/datasets/expected_outputs"
) -> Dict[str, Any]:
...
def evaluate_single_resume(
    pdf_path: str, expected_data: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    metrics["overall_accuracy"] = (
        round(total_matched / total_fields, 4) if total_fields > 0 else 0.0
    )

    return {"metrics": metrics, "actual_data": actual_data}
