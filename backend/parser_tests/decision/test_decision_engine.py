...
def test_decision_engine_strong_hire():
    score = ScoreResult(candidate_id="c1", job_id="j1", overall_score=90.0, category_scores={}, explanations=[])
    risk = RiskReport(candidate_id="c1", overall_risk_level="low", risks=[], confidence=1.0)
    
    result = decision_engine.make_decision(score, risk)
    assert result.decision == "Strong Hire"

def test_decision_engine_risk_penalty():
    score = ScoreResult(candidate_id="c1", job_id="j1", overall_score=80.0, category_scores={}, explanations=[])
    risk = RiskReport(candidate_id="c1", overall_risk_level="high", risks=[], confidence=1.0)
...
def test_decision_engine_critical_rejection():
    score = ScoreResult(candidate_id="c1", job_id="j1", overall_score=95.0, category_scores={}, explanations=[])
    risk = RiskReport(candidate_id="c1", overall_risk_level="critical", risks=[], confidence=1.0)
