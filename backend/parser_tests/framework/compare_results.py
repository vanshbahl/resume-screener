...
def load_results(
    run_id: str, results_dir: str = "parser_tests/results/benchmark_results"
):
...
def compare_runs(old_run: str, new_run: str):
    ...
    print("=== Parser Evaluation Comparison ===")
    ...
    print(
        f"Overall Accuracy: {o_acc*100:.2f}% -> {n_acc*100:.2f}% ({diff*100:+.2f}%) {icon}"
    )
    ...
    for r_name in new_resumes:
        ...
        for comp in ["experience", "projects", "education"]:
            ...
            if abs(o_acc - n_acc) > 0.01:
                has_changes = True
                report.append(
                    f"    - {comp.title()} Accuracy: {o_acc*100:.1f}% -> {n_acc*100:.1f}%"
                )
