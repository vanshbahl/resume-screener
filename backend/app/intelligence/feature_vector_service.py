...
class FeatureVectorService:
    ...
    def convert_resume_to_vector(self, parsed_resume: Dict[str, Any]) -> FeatureVector:
        ...
        return FeatureVector(
            ...
            frameworks=self._extract_normalized_list(parsed_resume.get("frameworks", [])),
            ...
            soft_skills=self._extract_normalized_list(parsed_resume.get("soft_skills", [])),
            
            years_experience=self._extract_years_experience(parsed_resume.get("experience", [])),
            education_level=self._extract_education_level(parsed_resume.get("education", [])),
            certifications=self._extract_normalized_list(parsed_resume.get("certifications", [])),
            
            # Additional heuristics can be applied here
            leadership_score=1.0 if "lead" in str(parsed_resume).lower() else 0.0,
            project_complexity_score=float(len(parsed_resume.get("projects", []))) * 0.5,
            technology_focus="specialist" if len(parsed_resume.get("skills", [])) < 5 else "generalist"
        )
        
    def convert_job_to_vectors(self, parsed_job: Dict[str, Any]) -> tuple[FeatureVector, FeatureVector]:
        ...
        req = FeatureVector(
            ...
            soft_skills=self._extract_normalized_list(parsed_job.get("soft_skills", [])),
            
            # Simple heuristic: JD experience usually comes as a single field
            years_experience=float(parsed_job.get("experience_requirements", 0.0) or 0.0),
