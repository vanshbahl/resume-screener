...
from parser_tests.dataset_generator.profiles.builder import ResumeProfileBuilder
from parser_tests.dataset_generator.templates.markdown import generate_markdown
from parser_tests.dataset_generator.templates.pdf import generate_pdf
from parser_tests.dataset_generator.metadata.ground_truth import generate_expected_json, generate_metadata_json, generate_notes_md
from parser_tests.dataset_generator.data.pools import EXPERIENCE_LEVELS

def ensure_dir(path: str):
    if not os.path.exists(path):
        os.makedirs(path)

def generate_resume_bundle(builder: ResumeProfileBuilder, output_dir: str, index: int, **kwargs):
    ...
    exp = profile["metadata"]["experience_level"].replace(" ", "_").replace("-", "_").lower()
    ...
    main_resumes = os.path.abspath(os.path.join(output_dir, "../../sample_resumes"))
    main_expected = os.path.abspath(os.path.join(output_dir, "../../expected_outputs"))
    
    ensure_dir(main_resumes)
    ensure_dir(main_expected)
    
    shutil.copy(pdf_path, os.path.join(main_resumes, f"{folder_name}.pdf"))
    with open(os.path.join(main_expected, f"{folder_name}.json"), "w", encoding="utf-8") as f:
        json.dump(expected, f, indent=2)
...
def main():
    ...
    parser.add_argument("--count", type=int, default=50, help="Number of resumes to generate")
    parser.add_argument("--industry", type=str, help="Force a specific industry")
    parser.add_argument("--experience", type=str, help="Force a specific experience level")
    ...
    exp_distribution = distribute_experience_levels(args.count) if not args.experience else [args.experience] * args.count
    
    print(f"Generating {args.count} realistic resumes...")
    
    for i in range(args.count):
        kwargs = {}
        if args.industry: kwargs["industry"] = args.industry
    ...
    print("PDFs and Expected JSONs have been automatically linked to the benchmark suite directories.")
