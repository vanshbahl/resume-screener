...
from app.job.schemas.job import (HiringTeamUpdate, JobCreate, JobStatusUpdate,
                                 JobTagsUpdate, JobUpdate)
from app.job.services.timeline_service import timeline_service


class JobService:
    def create_job(
        self, db: Session, job_in: JobCreate, user_id: Optional[str] = None
    ) -> Job:
        obj_in = {
            "title": job_in.title,
    ...
    def update_job(
        self, db: Session, job_id: str, job_in: JobUpdate, user_id: Optional[str] = None
    ) -> Optional[Job]:
