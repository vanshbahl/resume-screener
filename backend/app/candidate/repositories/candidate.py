...
from app.candidate.models.candidate import (Candidate, CandidateAttachment,
                                            CandidateNote, CandidateResume,
                                            CandidateTimeline)
...
class CandidateResumeRepository(BaseRepository[CandidateResume]):
    ...
    def get_active_resume(
        ...
        return (
            db.query(self.model)
            .filter(
                self.model.candidate_id == candidate_id, self.model.is_active == True
            )
            .first()
        )
