...
class SpacyNERStage(BaseParserStage):
    def run(self, document: BaseDocument, context: PipelineContext) -> None:
        ...
        if not nlp:
            context.log_warning(
                "SpacyLoadError", "Spacy model not loaded, skipping stage."
            )
            return
        ...
        entities = {"PERSON": [], "ORG": [], "GPE": [], "DATE": [], "WORK_OF_ART": []}
        ...
        document.metadata["model_versions"]["spacy"] = getattr(
            nlp, "name", "en_core_web_trf"
        )
