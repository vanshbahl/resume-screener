...
class ContactExtractor:
    ...
    def _create_field(
        self, value: Any, conf: float, source_line: Dict, section: str = "personal_info"
    ) -> Dict:
        if not value:
            return None
        return {
    ...
    def extract(self, lines: List[Dict]) -> Dict:
        ...
        for line_obj in lines:
            ...
            if not result["phone"]:
                ...
                if match:
                    result["phone"] = self._create_field(
                        match.group(0).strip(), 0.95, line_obj
                    )
            ...
            if not result["portfolio"]:
                ...
                for match in self.bare_host_regex.finditer(text):
                    ...
                    if not re.search(
                        r"\.(com|io|dev|net|org|me|co|in|app|tech|xyz|ai)(/|$)",
                        candidate,
                        re.IGNORECASE,
                    ):
                        continue
            ...
            if not result["location"]:
                ...
                if match and "university" not in match.group(0).lower():
                    result["location"] = self._create_field(
                        match.group(0), 0.8, line_obj
                    )
