...
from app.schemas.intelligence import MatchResult, ScoreResult
...
class ScoringService:
    ...
    def score_match(self, match_result: MatchResult) -> ScoreResult:
        ...
        for exp in match_result.explanations:
            if exp.category == "experience":
                if exp.missing_entities:
                    ...
                    matched_yoe = (
                        float(exp.matched_entities[0]) if exp.matched_entities else 0.0
                    )
                    if missing_yoe > 0:
                        exp_score = min(
                            100.0, max(0.0, (matched_yoe / missing_yoe) * 100)
                        )
        ...
        category_scores["projects"] = (
            100.0  # Placeholder: Would calculate from project complexity
        )
        category_scores["education"] = (
            100.0  # Placeholder: Would calculate from education level reqs
        )
