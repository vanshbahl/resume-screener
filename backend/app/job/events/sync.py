...
def sync_job_intelligence(db: Session, job_id: str):
    ...
    if (
        not active_jd
        or not active_jd.parsed_metadata
        or "structured_data" not in active_jd.parsed_metadata
    ):
        logger.warning(
            f"Job {job_id} has no processed active JD to sync. Will sync base metadata only."
        )
        raw_data = {}
    else:
