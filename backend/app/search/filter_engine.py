...
class FilterEngine:
    ...
    def apply_filters(
        self, context: SearchContext, candidate_ids: Set[str]
    ) -> Set[str]:
        ...
        for doc_id in candidate_ids:
            ...
            if context.query.experience:
                if context.query.experience.min is not None:
                    if (
                        getattr(doc, "years_experience", 0.0)
                        < context.query.experience.min
                    ):
                        passed = False
                if context.query.experience.max is not None:
                    if (
                        getattr(doc, "years_experience", 0.0)
                        > context.query.experience.max
                    ):
                        passed = False
            ...
            if context.query.education_levels and passed:
                if (
                    getattr(doc, "education_level", "none")
                    not in context.query.education_levels
                ):
                    passed = False
        ...
        if initial_count > 0:
            context.metrics.filter_drop_rate = (
