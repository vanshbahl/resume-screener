...
class MatchingService:
    ...
    def _compare_lists(
        self, candidate_items: List[str], required_items: List[str], category_name: str
    ) -> Explanation:
            ...
            if req_family:
                for cand in cand_set:
                    if self.ontology.get_semantic_family(cand) == req_family:
                        partial_matches.add(
                            f"{req} (Matched via {cand} in family {req_family})"
                        )
                        resolved = True
                        break
        ...
        reason = (
            f"Matched {len(exact_matches)} exactly, {len(partial_matches)} partially."
        )
        if still_missing:
    ...
    def match(
        ...
        candidate_id: str,
        job_id: str,
        candidate_vec: FeatureVector,
        req_vec: FeatureVector,
        pref_vec: FeatureVector = None,
    ) -> MatchResult:
        ...
        explanations.append(
            Explanation(
                ...
                missing_entities=(
                    []
                    if candidate_vec.years_experience >= req_vec.years_experience
                    else [str(req_vec.years_experience)]
                ),
                reason=exp_reason,
