...
from app.schemas.intelligence import RankedItem, RankingResult, ScoreResult


class RankingService:
    def rank_candidates_for_job(
        self, job_id: str, score_results: List[ScoreResult]
    ) -> RankingResult:
        """Ranks a list of ScoreResults for a specific job."""

        # Sort by overall score descending
        sorted_scores = sorted(
            score_results, key=lambda x: x.overall_score, reverse=True
        )
        ...
        return RankingResult(
            ranking_context=f"Candidates for Job {job_id}", rankings=rankings
        )

    def rank_generic(
        self, items: List[Dict[str, float]], context: str
    ) -> RankingResult:
        """
        ...
        return RankingResult(ranking_context=context, rankings=rankings)
