...
from app.schemas.decision import (CareerRecommendation, HiringRecommendation,
                                  LearningRecommendation,
                                  RiskReport)
from app.schemas.intelligence import CandidateProfile, GapAnalysis, ScoreResult
...
class HiringRecommendationService:
    def __init__(self):
        ...
        self.rules = (
            config.get("hiring_categories", {}) if isinstance(config, dict) else {}
        )

    def categorize_candidate(
        self, score_res: ScoreResult, risk: RiskReport
    ) -> HiringRecommendation:
        top = self.rules.get("top_candidate", {"min_score": 80.0})
        gem = self.rules.get(
            "hidden_gem",
            {"min_score": 55.0, "required_insight": "Leadership Potential"},
        )
        ...
        if score_res.overall_score >= top.get(
            "min_score", 80.0
        ) and risk.overall_risk_level in ["low", "medium"]:
            ...
            score_res.overall_score >= gem.get("min_score", 55.0)
