...
def generate_markdown(profile: Dict[str, Any]) -> str:
    ...
    if pi.get("github"): contact.append(pi["github"])
    if pi.get("linkedin"): contact.append(pi["linkedin"])
    if pi.get("portfolio"): contact.append(pi["portfolio"])
    ...
    if profile.get("skills"):
        md.append("## Skills")
        s = profile["skills"]
        if s.get("languages"): md.append(f"**Languages:** {', '.join(s['languages'])}")
        if s.get("frameworks"): md.append(f"**Frameworks:** {', '.join(s['frameworks'])}")
        if s.get("tools"): md.append(f"**Tools:** {', '.join(s['tools'])}")
