...
from app.job.schemas.job import (HiringTeamUpdate, JobCreate,
                                 JobDescriptionResponse, JobNoteCreate,
                                 JobNoteResponse, JobResponse, JobStatusUpdate,
                                 JobTagsUpdate, JobTimelineResponse, JobUpdate)
...
def update_hiring_team(
    job_id: str, team_update: HiringTeamUpdate, db: Session = Depends(get_db)
):
...
async def upload_job_description(
    ...
    try:
        jd = job_description_service.upload_description(
            db, job_id, file_bytes, file.filename, file.content_type
        )
        # Background task for parsing
        background_tasks.add_task(
            job_description_service.process_description, next(get_db()), jd.id
        )
        return jd
...
def add_note(job_id: str, note_in: JobNoteCreate, db: Session = Depends(get_db)):
    ...
    return note_service.add_note(
        db, job_id, note_in, author="Current User"
