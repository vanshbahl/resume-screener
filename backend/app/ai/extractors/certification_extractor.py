...
class CertificationExtractor:
    ...
    def extract(self, lines: List[Dict]) -> List[Dict]:
        ...
        for blk in blocks:
            ...
            if blk:
                entry["name"] = self._create_field(
                    blk[0]["text"].lstrip("-•* "), 0.8, blk[0]
                )
            ...
            for line in blk:
                ...
                issuer_match = re.search(
                    r"\b(AWS|Google|Microsoft|Coursera|Udemy|Oracle|Cisco|IBM|CompTIA)\b",
                    text,
                    re.IGNORECASE,
                )
                if issuer_match and "issuer" not in entry:
                    entry["issuer"] = self._create_field(
                        issuer_match.group(1), 0.9, line
                    )
