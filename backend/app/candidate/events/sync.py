...
def sync_candidate_intelligence(db: Session, candidate_id: str):
    ...
    if (
        not active_resume
        or not active_resume.parsed_metadata
        or "structured_data" not in active_resume.parsed_metadata
    ):
        logger.warning(
            f"Candidate {candidate_id} has no processed active resume to sync."
        )
        return
    ...
    try:
        ...
        logger.error(
            f"Failed to generate feature vector for candidate {candidate_id}: {e}"
        )
        return
    ...
    profile = CandidateProfile(
        candidate_id=candidate.id, raw_data=raw_data, vector=vector
    )
