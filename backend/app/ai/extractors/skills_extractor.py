...
class SkillsExtractor:
    def _create_field(
        self, value: Any, conf: float, source_line: Dict, section: str = "skills"
    ) -> Dict:
        return {
            "value": value,
            "confidence": conf,
    ...
    def extract(
        self, lines: List[Dict], context: PipelineContext
    ) -> Dict[str, List[Dict]]:
        ...
        for cat, items in kb.items():
            ...
            for normalized, aliases in items.items():
                ...
                kb_regexes[(schema_cat, normalized)] = re.compile(
        ...
        for line_obj in lines:
            ...
            for (schema_cat, normalized), regex in kb_regexes.items():
                ...
                if match:
                    ...
                    result[schema_cat].append(
                        self._create_field(normalized, 0.95, line_obj)
                    )
                    result["skills"].append(
                        self._create_field(normalized, 0.95, line_obj)
                    )  # Also add to generic skills pool
