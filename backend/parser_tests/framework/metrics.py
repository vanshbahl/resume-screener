...
def calculate_field_match(
    expected: Any, actual: Any, fuzzy_threshold: float = 90.0
) -> bool:
...
def calculate_list_metrics(
    expected_list: List[Any], actual_list: List[Any], fuzzy_threshold: float = 90.0
) -> Dict[str, Any]:
    ...
    for exp in exp_vals:
        ...
        for i, act in enumerate(act_vals):
            if i not in matched_actuals and calculate_field_match(
                exp, act, fuzzy_threshold
            ):
                tp += 1
...
def evaluate_complex_list(
    expected_items: List[Dict], actual_items: List[Dict], match_keys: List[str]
) -> Dict[str, Any]:
