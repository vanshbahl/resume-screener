...
class AchievementExtractor:
    def _create_field(self, value: Any, conf: float, source_line: Dict, section: str = "achievements") -> Dict:
    ...
    def extract(self, lines: List[Dict]) -> List[Dict]:
        ...
        for blk in blocks:
            ...
            for line in blk:
                ...
                rank_match = re.search(r'\b(1st|2nd|3rd|4th|[0-9]+th|winner|runner up|finalist)\b', text, re.IGNORECASE)
                if rank_match and "rank" not in entry:
                    entry["rank"] = self._create_field(rank_match.group(1), 0.9, line)
                    
                award_match = re.search(r'\b(award|prize|scholarship|medal|honorable mention)\b', text, re.IGNORECASE)
                if award_match and "award" not in entry:
                    entry["award"] = self._create_field(text, 0.7, line) # the whole text might be the award name
                    
                comp_match = re.search(r'\b(hackathon|competition|olympiad|championship|tournament)\b', text, re.IGNORECASE)
