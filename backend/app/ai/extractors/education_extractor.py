...
class EducationExtractor:
    def _create_field(self, value: Any, conf: float, source_line: Dict, section: str = "education") -> Dict:
        ...
        for blk in blocks:
            ...
            for line in blk:
                ...
                if match and "start_date" not in entry:
                    ...
                    if "expected" in text.lower() or "present" in match.group(2).lower() or "now" in match.group(2).lower():
                        entry["expected_graduation"] = self._create_field(match.group(2), 0.85, line)
                    else:
                        entry["graduation_year"] = self._create_field(match.group(2), 0.85, line)
                ...
                if single_year_match and not match and "graduation_year" not in entry:
                    if "expected" in text.lower():
                        entry["expected_graduation"] = self._create_field(single_year_match.group(1), 0.8, line)
                    else:
                        entry["graduation_year"] = self._create_field(single_year_match.group(1), 0.8, line)
                ...
                if percentage_match and "percentage" not in entry:
                    entry["percentage"] = self._create_field(percentage_match.group(1), 0.9, line)
                
                semester_match = re.search(r'(\d(?:st|nd|rd|th)\s*semester)', text, re.IGNORECASE)
                if semester_match and "current_semester" not in entry:
                    entry["current_semester"] = self._create_field(semester_match.group(1), 0.9, line)
                ...
                deg_keywords = ["b.s.", "b.a.", "bachelor", "master", "m.s.", "m.a.", "phd", "ph.d", "degree", "b.tech", "m.tech", "diploma"]
                if any(k in text.lower() for k in deg_keywords) and "degree" not in entry:
                    ...
                    deg_match = re.search(r'([A-Za-z\.\s]+)\s+in\s+([A-Za-z\s]+)', text, re.IGNORECASE)
                    if deg_match:
                        entry["degree"] = self._create_field(deg_match.group(1).strip(), 0.85, line)
                        entry["field_of_study"] = self._create_field(deg_match.group(2).strip(), 0.85, line)
                    else:
                        entry["degree"] = self._create_field(text, 0.8, line)
                        
                loc_match = re.search(r'([A-Z][a-zA-Z\s]+),\s*([A-Z]{2}|[A-Z][a-zA-Z\s]+)', text)
                if loc_match and "location" not in entry and "university" not in loc_match.group(0).lower():
