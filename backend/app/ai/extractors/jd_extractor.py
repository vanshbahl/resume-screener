...
class JDExtractor:
    def __init__(self):
        config_path = os.path.join(
            os.path.dirname(__file__), "../../../config/jd_patterns.yaml"
        )
        self.patterns = {}
    ...
    def extract(self, cleaned_lines: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if exp_cfg and "pattern" in exp_cfg:
            ...
            if match:
                result["experience_requirements"] = self._create_field(
                    match.group(0), first_line_source
                )
        ...
        if emp_cfg and "patterns" in emp_cfg:
            for pat in emp_cfg["patterns"]:
                if re.search(r"\b" + pat + r"\b", full_text):
                    result["employment_type"] = self._create_field(
                        pat.replace("\\", ""), first_line_source
                    )
                    break
        ...
        if loc_cfg and "patterns" in loc_cfg:
            for pat in loc_cfg["patterns"]:
                if re.search(r"\b" + pat + r"\b", full_text):
                    result["location_type"] = self._create_field(
                        pat.replace("\\", ""), first_line_source
                    )
                    break
        ...
        if deg_cfg and "patterns" in deg_cfg:
            for pat in deg_cfg["patterns"]:
                if re.search(r"\b" + pat + r"\b", full_text):
                    result["degree_requirements"].append(
                        self._create_field(
                            pat.replace("\\", "").replace("?", ""), first_line_source
                        )
                    )
        ...
        if visa_cfg and "patterns" in visa_cfg:
            for pat in visa_cfg["patterns"]:
                if re.search(r"\b" + pat + r"\b", full_text):
                    result["visa_requirements"].append(
                        self._create_field(pat.replace("\\", ""), first_line_source)
                    )
