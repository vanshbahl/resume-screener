...
class ExperienceExtractor:
    ...
    def _create_field(
        self, value: Any, conf: float, source_line: Dict, section: str = "experience"
    ) -> Dict:
        if not value:
            return None
        return {
        ...
        for blk in blocks:
            ...
            for i, line in enumerate(blk):
                ...
                if match and "start_date" not in entry:
                    ...
                    dur_match = re.search(
                        r"\((.*?(?:year|month).*?)\)", text, re.IGNORECASE
                    )
                    if dur_match:
                ...
                for et in emp_type_keywords:
                    if et in text.lower() and "employment_type" not in entry:
                        entry["employment_type"] = self._create_field(
                            et.title(), 0.9, line
                        )
                ...
                if (
                    ...
                    responsibilities.append(
                        self._create_field(text.strip().lstrip("-•* "), 0.9, line)
                    )
            ...
            for i, line in enumerate(blk[:3]):
                ...
                if (
                    any(k in text.lower() for k in title_keywords)
                    ...
                    entry["company"] = self._create_field(
                        text, 0.7, line
                    )  # Confidence lower, let AI fusion fix this if needed
