...
class InsightService:
    def generate_insights(self, candidate_vec: FeatureVector) -> List[Insight]:
        ...
        # Total skills metric
        total_tech_skills = (
            len(candidate_vec.skills)
            + len(candidate_vec.frameworks)
            + len(candidate_vec.tools)
            + len(candidate_vec.languages)
            + len(candidate_vec.databases)
            + len(candidate_vec.cloud)
        )
        ...
        if (
            candidate_vec.years_experience > 5.0
            and candidate_vec.leadership_score > 0.0
        ):
            insights.append(
                Insight(
