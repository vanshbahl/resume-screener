...
from app.schemas.decision import DecisionResult, RiskItem, RiskReport
from app.schemas.intelligence import (CandidateProfile, Explanation,
                                      JobProfile, ScoreResult)
...
class RiskAnalysisService(BaseRiskAnalysisService):
    ...
    def analyze_risks(
        ...
        if (
            candidate.vector.years_experience > 0
            and len(candidate.vector.skills) > 30
            and candidate.vector.years_experience < 2.0
        ):
            risks.append(
                RiskItem(
        ...
        if job and job.required_vector.years_experience > 0:
            excess = (
                candidate.vector.years_experience - job.required_vector.years_experience
            )
...
class DecisionEngine(BaseDecisionEngine):
    ...
    def make_decision(
        ...
        if risk_report:
            if risk_report.overall_risk_level == "critical" and self.adjustments.get(
                "critical_risk_rejection"
            ):
                decision = "Reject"
                ...
                return DecisionResult(
                    ...
                    evidence=[
                        Explanation(
                            ...
                            confidence=1.0,
                            reason=reason,
                            evidence="Critical risk detected",
                        )
                    ],
