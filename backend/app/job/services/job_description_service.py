...
class JobDescriptionService:
    def upload_description(self, db: Session, job_id: str, file_bytes: bytes, filename: str, content_type: str, user_id: Optional[str] = None) -> JobDescription:
        ...
        jd = job_description_repo.create(db, {
            "job_id": job_id,
    ...
    def process_description(self, db: Session, jd_id: str):
        ...
        try:
            ...
            import copy
            new_metadata = copy.deepcopy(jd.parsed_metadata) if jd.parsed_metadata else {}
            new_metadata["structured_data"] = structured_data
            new_metadata["raw_text"] = clean_pdf_text
            
            jd = job_description_repo.update(db, jd, {
                "parsed_metadata": new_metadata,
                "parser_version": "2.2.5"
            })
