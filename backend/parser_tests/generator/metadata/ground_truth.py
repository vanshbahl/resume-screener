...
def _wrap_field(value: Any, section: str = None) -> Any:
    ...
    if isinstance(value, list) and all(isinstance(v, str) for v in value):
        return [{"value": v, "confidence": 1.0, "source": {"page": 1, "section": section, "line": 0}, "origin_model": "ground_truth"} for v in value]
...
def generate_expected_json(profile: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for exp in profile.get("experience", []):
        exp_list.append({
            ...
            "responsibilities": _wrap_field(exp.get("responsibilities", []), "experience")
        })
    ...
    expected["metadata"] = {
        ...
        "sections_detected": ["personal_info", "experience", "projects", "education", "skills"],
        "parsing_time_ms": 0,
...
def generate_notes_md(profile: Dict[str, Any]) -> str:
    ...
    if not meta.get("has_summary"): notes.append("- No summary block")
    if not meta.get("has_projects"): notes.append("- No projects section")
    if not meta.get("has_experience"): notes.append("- No experience section (Fresher layout)")
