...
def test_risk_analysis_overqualification():
    ...
    job = JobProfile(job_id="j1", raw_text="", raw_data={}, required_vector=job_vec, preferred_vector=FeatureVector())
...
def test_risk_analysis_skill_mismatch():
    ...
    job_vec = FeatureVector(years_experience=2.0, skills=["Python", "AWS", "Docker", "React"])
    job = JobProfile(job_id="j1", raw_text="", raw_data={}, required_vector=job_vec, preferred_vector=FeatureVector())
