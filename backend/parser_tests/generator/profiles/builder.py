...
class ResumeProfileBuilder:
    ...
    def generate_profile(self, **kwargs) -> Dict[str, Any]:
        ...
        has_projects = kwargs.get(
            "has_projects",
            exp_level in ["No experience", "Internship", "0-2 years"]
        ...
        skills_count = (
            random.randint(5, 12)
            if kwargs.get("many_skills", False)
        ...
        profile = {
            ...
            "experience": (
                self._generate_experience(exp_level, industry) if has_experience else []
            ),
    ...
    def _generate_personal_info(self, country: str) -> Dict[str, Any]:
        ...
        github = (
            f"github.com/{first_name.lower()}{random.randint(1,99)}"
        ...
        portfolio = (
            f"{first_name.lower()}{last_name.lower()}.dev"
    ...
    def _generate_skills(self, count: int) -> Dict[str, List[str]]:
        langs = random.sample(
            pools.TECH_STACK["languages"],
            min(count, len(pools.TECH_STACK["languages"])),
        )
        frameworks = random.sample(
            pools.TECH_STACK["frameworks"],
            min(count - 1, len(pools.TECH_STACK["frameworks"])),
        )
        tools = random.sample(
            pools.TECH_STACK["tools"], min(count - 2, len(pools.TECH_STACK["tools"]))
        )
    ...
    def _generate_education(self, exp_level: str) -> List[Dict[str, Any]]:
        edu = []
        edu_count = (
            1 if exp_level in ["5-10 years", "10-20 years"] else random.randint(1, 2)
        )
    ...
    def _generate_experience(
        self, exp_level: str, industry: str
    ) -> List[Dict[str, Any]]:
        count_map = {
            "Internship": 1,
        ...
        for i in range(exp_count):
            ...
            for _ in range(resp_count):
                ...
                noun = self.fake.bs()
                metric = (
                    f" by {random.randint(10, 50)}%" if random.random() > 0.5 else ""
                )
            ...
            exp.append(
                {
                    ...
                    "end_date": (
                        "Present" if i == 0 else f"{self.fake.month_name()} {end_yr}"
                    ),
    ...
    def _generate_projects(self, industry: str) -> List[Dict[str, Any]]:
        ...
        for _ in range(proj_count):
            ...
            tech = random.sample(
                pools.TECH_STACK["languages"] + pools.TECH_STACK["frameworks"],
                tech_count,
            )
            ...
            desc = " ".join(
                [
                    f"{random.choice(pools.PROJECT_VERBS)} {self.fake.bs()}."
            ...
            projects.append(
                {
                    ...
                    "description": desc,
                    "link": (
                        f"github.com/fake/{name.replace(' ', '').lower()}"
                        if random.random() > 0.5
                        else None
                    ),
                }
