...
def generate_reports(
    ...
    output_dir: str = "parser_tests/results/benchmark_results",
):
...
def generate_markdown_summary(run_id: str, results: Dict[str, Any], run_dir: str):
    ...
    for r_name, r_data in resumes.items():
        ...
        lines.append(
            f"**Accuracy**: {metrics.get('overall_accuracy', 0.0) * 100:.1f}%  "
        )
...
def generate_accuracy_csv(resume_metrics: Dict[str, Any], run_dir: str):
    ...
    with open(
        os.path.join(run_dir, "accuracy_table.csv"), "w", newline="", encoding="utf-8"
    ) as f:
        writer = csv.writer(f)
...
def generate_charts(resume_metrics: Dict[str, Any], run_dir: str):
    ...
    accuracies = [
        m["metrics"].get("overall_accuracy", 0.0) * 100 for m in resume_metrics.values()
    ]
