...
class EntityFusionStage(BaseParserStage):
    ...
    def _fuse_jd(self, document: JobDocument, context: PipelineContext) -> None:
        ...
        document.metadata["entities_added_by_ai"] = (
            document.metadata.get("entities_added_by_ai", 0) + entities_added
        )
        document.metadata["entities_modified_by_ai"] = (
            document.metadata.get("entities_modified_by_ai", 0) + entities_modified
        )
    ...
    def _fuse_resume(self, document: BaseDocument, context: PipelineContext) -> None:
        ...
        for exp in extracted.get("experience", []):
            if not exp.get("company"):
                ...
                for ent in spacy_ents.get("ORG", []):
                    if (
                        start_line <= ent["source"]["line"] <= end_line
                        and ent["confidence"] > best_conf
                    ):
                        best_org = ent
                        best_conf = ent["confidence"]
                        best_source = "spacy"
                ...
                start_line = (
                    exp["company"]["source"]["line"]
                    if exp.get("company") and exp["company"].get("source")
                    else 1
                )

                for ent in hf_ents.get("ORG", []) + spacy_ents.get("ORG", []):
                    if ent["source"]["line"] == start_line and ent["confidence"] > 0.85:
                        if det_comp in ent["value"] and len(ent["value"]) > len(
                            det_comp
                        ):
