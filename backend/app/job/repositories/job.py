...
class JobDescriptionRepository(BaseRepository[JobDescription]):
    ...
    def get_active_description(self, db: Session, job_id: str) -> Optional[JobDescription]:
    ...
    def deactivate_all_for_job(self, db: Session, job_id: str):
        db.query(self.model).filter(self.model.job_id == job_id).update({"is_active": False})
        db.commit()
